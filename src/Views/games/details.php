...
<body class="bg-zinc-950 text-zinc-200 font-sans min-h-screen pb-12 selection:bg-violet-600 selection:text-white">
            ...
            <div class="p-6 md:p-8 flex-1 flex flex-col">
                ...
                <div class="relative">
                    <div id="descriptionContainer" class="game-description text-sm sm:text-base pr-2 max-h-56 overflow-hidden transition-[max-height] duration-500 ease-in-out">
                        <?php echo $gameDetails['description'] ?: '<p class="text-zinc-500 italic">Nenhuma descrição disponível para este jogo.</p>'; ?>
                    </div>
        ...
        <div>
            ...
            <form action="index.php?action=save_review" method="post" class="bg-zinc-900 p-6 sm:p-8 rounded-sm border-2 border-zinc-800 shadow-xl">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="review" name="review" rows="6" placeholder="Escreva suas anotações, pensamentos ou a sua resenha crítica sobre este jogo..."
                        class="w-full bg-zinc-950 border-2 border-zinc-800 text-white rounded-sm px-5 py-4 focus:outline-none focus:border-violet-500 font-medium placeholder-zinc-600 resize-y"><?php echo isset($userGameInfo['review']) ? htmlspecialchars($userGameInfo['review']) : ''; ?></textarea>
                </div>
    ...
    <script>
        ...
        Toast.fire({
            icon: 'success',
            title: 'Resenha salva com sucesso!'
        });
    </script>
